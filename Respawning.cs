...
public class Respawning// : IRespawning
{
    ...
    public void SurrenderToPolice(ILocationRespawnable respawnableLocation)
    {
        ...
        // Move player to the prison transport bus spawn point (still faded out).
        // The gate transport will drive them through the perimeter from here.
        NativeFunction.Natives.SET_ENTITY_COORDS_NO_OFFSET(
            Game.LocalPlayer.Character,
            1971.267f, 2635.439f, 46.17389f,   // PrisonGateTransportActivity.BusSpawnPos
            false, false, false);
        NativeFunction.Natives.SET_ENTITY_HEADING(Game.LocalPlayer.Character, 120.792f);

        // Start the real-world sentence timer — it runs during transport and inside
        ...
        // Player is driven through the two Bolingbroke security gates by a guard.
        var transport = new PrisonGateTransportActivity();
        bool transportOk = transport.Start();  // blocks this fiber until transport completes

        if (!transportOk)
        {
            // Fallback: transport failed/timed out — drop player inside directly
            EntryPoint.WriteToConsole("[SurrenderToPolice] Gate transport failed, using fallback teleport.", 0);
            NativeFunction.Natives.SET_ENTITY_COORDS_NO_OFFSET(
                Game.LocalPlayer.Character,
                1617.643f, 2523.981f, 45.56489f,
                false, false, false);
            NativeFunction.Natives.SET_ENTITY_HEADING(Game.LocalPlayer.Character, 264.718f);
        }

        // Apply prison outfit now that the player is inside the perimeter
        BolingbrookeOutfit.Apply(Game.LocalPlayer.Character);

        // Freeze the player initially; starting a riot unfreezes them
        Rage.Game.LocalPlayer.Character.IsPositionFrozen = true;
        Rage.Game.LocalPlayer.Character.BlockPermanentEvents = true;

        var riot = new PrisonRiotActivity();
        bool riotStarted = false;
        uint lastDisplayTick = 0;

        Player.ButtonPrompts.AddPrompt("SentencePD", "Start Riot ~r~($5,000,000)~s~", "TriggerRiot", GameControl.Jump, 14);

        while (!_prisonSentenceTracker.TryRelease())
        {
            // --- Riot trigger ---
            if (!riotStarted && Player.ButtonPrompts.IsPressed("TriggerRiot"))
            {
                if (Player.BankAccounts.GetMoney(false) >= 5_000_000)
                {
                    Player.BankAccounts.GiveMoney(-5_000_000, false);
                    Rage.Game.LocalPlayer.Character.IsPositionFrozen = false;
                    Rage.Game.LocalPlayer.Character.BlockPermanentEvents = false;
                    riot.Start();
                    riotStarted = true;
                    Player.ButtonPrompts.RemovePrompts("SentencePD");
                }
                else
                {
                    Rage.Game.DisplayNotification(
                        "CHAR_CALL911", "CHAR_CALL911",
                        "Bolingbroke Penitentiary", "~r~Insufficient Funds",
                        "Starting a riot costs ~r~$5,000,000~s~ cash on hand.");
                }
            }

            // Countdown is shown only while frozen (riot has its own notifications)
            if (!riotStarted && (Rage.Game.GameTime - lastDisplayTick >= 1000 || lastDisplayTick == 0))
            {
        ...
        // Restore civilian clothes on release
        BolingbrookeOutfit.Remove(Game.LocalPlayer.Character);
